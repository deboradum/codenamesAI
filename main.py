...
def get_log_path(log_dir="logs/"):
    ...
    game_numbers = [
        int(f[5:-5]) for f in os.listdir(log_dir) if f.startswith("game_") and f[5:-5].isdigit()
    ]
    next_number = max(game_numbers, default=0) + 1

    return os.path.join(log_dir, f"game_{next_number}.json")
...
if __name__ == "__main__":
    ...
    for simulation in range(args.num_simulations):
        ...
        log_data = {
            ...
            "word_assignments": game.original_word_assignments,
